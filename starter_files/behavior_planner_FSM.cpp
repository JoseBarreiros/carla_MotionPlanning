...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = std::abs((velocity_mag * velocity_mag) / (2.0 * accel_mag));  // <- Fix This

  LOG(INFO) << "Calculated look_ahead_distance: " << look_ahead_distance << "acc: " <<accel_mag << "velo: " << velocity_mag;
